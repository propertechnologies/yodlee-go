...
// swagger:model DocumentDownloadResponse
...
func (m *DocumentDownloadResponse) validateDocument(formats strfmt.Registry) error {

	if swag.IsZero(m.Document) { // not required
		return nil
	}

	for i := 0; i < len(m.Document); i++ {
		if swag.IsZero(m.Document[i]) { // not required
			continue
		}

		if m.Document[i] != nil {
			if err := m.Document[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("document" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *DocumentDownloadResponse) UnmarshalBinary(b []byte) error {
	var res DocumentDownloadResponse
